...
export function parseVCAPServices(): BTPConfig | null {
  ...
  getLogger().info('BTP VCAP_SERVICES parsed', {
...
export async function lookupDestination(btpConfig: BTPConfig, destinationName: string): Promise<Destination> {
  ...
  getLogger().info('BTP destination resolved', {
...
export async function lookupDestinationWithUserToken(
  ...
  try {
    ...
    if (parts.length === 3) {
      ...
      getLogger().debug('PP user JWT claims', {
    ...
    getLogger().debug('PP user JWT: failed to decode claims');
  }
  ...
  getLogger().debug('Destination Service PP response', {
    destination: destinationName,
  ...
  if (sdkAuthTokens) {
    for (const token of sdkAuthTokens) {
      if (token.error) {
        getLogger().error('Destination Service auth token error', {
          destination: destinationName,
      ...
      if (token.http_header?.key === 'SAP-Connectivity-Authentication') {
        ...
        getLogger().debug('PP: SAP-Connectivity-Authentication header extracted', {
          destination: destinationName,
    ...
    getLogger().warn('Destination Service returned no authTokens — trying jwt-bearer exchange fallback', {
      destination: destinationName,
      authentication: dest.Authentication,
    });
  }
  ...
  if (!tokens.sapConnectivityAuth && dest.Authentication === 'PrincipalPropagation' && btpConfig.connectivityClientId) {
    getLogger().info('PP jwt-bearer exchange: attempting direct exchange with Connectivity Service', {
      destination: destinationName,
    ...
    try {
      ...
      if (exchangeResp.ok) {
        ...
        getLogger().info('PP: using Option 2 (SAP-Connectivity-Authentication with original JWT)', {
          destination: destinationName,
        });
      } else {
        const errText = await exchangeResp.text();
        getLogger().error('PP jwt-bearer exchange: failed', {
          destination: destinationName,
      ...
      getLogger().error('PP jwt-bearer exchange: error', {
        destination: destinationName,
  ...
  getLogger().info('BTP destination resolved (per-user)', {
    name: dest.Name,
    url: dest.URL,
